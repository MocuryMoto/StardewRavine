package net.mocury.stardewravine.datagen;

import net.fabricmc.fabric.api.datagen.v1.FabricDataOutput;
import net.fabricmc.fabric.api.datagen.v1.provider.FabricTagProvider;
import net.minecraft.registry.RegistryWrapper;
import net.minecraft.registry.tag.BlockTags;
import net.mocury.stardewravine.block.ModBlocks;
import net.mocury.stardewravine.util.ModTags;

import java.util.concurrent.CompletableFuture;

public class ModBlockTagProvider extends FabricTagProvider.BlockTagProvider {

    public ModBlockTagProvider(FabricDataOutput output, CompletableFuture<RegistryWrapper.WrapperLookup> registriesFuture) {
        super(output, registriesFuture);
    }

    @Override
    protected void configure(RegistryWrapper.WrapperLookup lookup) {
        getOrCreateTagBuilder(BlockTags.PICKAXE_MINEABLE)
                .add(ModBlocks.AQUAMARINE_BLOCK)
                .add(ModBlocks.AQUAMARINE_ORE)
                .add(ModBlocks.AQUAMARINE_DEEPSLATE_ORE)
                .add(ModBlocks.TOPAZ_BLOCK)
                .add(ModBlocks.TOPAZ_ORE)
                .add(ModBlocks.TOPAZ_DEEPSLATE_ORE)
                .add(ModBlocks.NETHER_TOPAZ_ORE)
                .add(ModBlocks.JADE_BLOCK)
                .add(ModBlocks.JADE_ORE)
                .add(ModBlocks.JADE_DEEPSLATE_ORE)
                .add(ModBlocks.RUBY_BLOCK)
                .add(ModBlocks.RUBY_ORE)
                .add(ModBlocks.RUBY_DEEPSLATE_ORE)
                .add(ModBlocks.NETHER_RUBY_ORE)
                .add(ModBlocks.NETHER_RADIONITE_ORE)
                .add(ModBlocks.RADIONITE_BLOCK)
                .add(ModBlocks.RAW_RADIONITE_BLOCK)
                .add(ModBlocks.PRISMATIC_SHARD_BLOCK)
                .add(ModBlocks.REFINED_QUARTZ_BLOCK)
                .add(ModBlocks.IRIDIUM_BLOCK)
                .add(ModBlocks.RAW_IRIDIUM_BLOCK)
                .add(ModBlocks.IRIDIUM_DEEPSLATE_ORE)
                .add(ModBlocks.NETHER_IRIDIUM_ORE)
                .add(ModBlocks.ESPERITE)
                .add(ModBlocks.POLISHED_ESPERITE)
                .add(ModBlocks.SMOOTH_ESPERITE)
                .add(ModBlocks.ESPERITE_BRICKS)
                .add(ModBlocks.ESPERITE_PILLAR)
                .add(ModBlocks.CHISELED_ESPERITE)
                .add(ModBlocks.MARBLE)
                .add(ModBlocks.POLISHED_MARBLE)
                .add(ModBlocks.SMOOTH_MARBLE)
                .add(ModBlocks.MARBLE_BRICKS)
                .add(ModBlocks.MARBLE_PILLAR)
                .add(ModBlocks.CHISELED_MARBLE)
                .add(ModBlocks.SOAPSTONE)
                .add(ModBlocks.POLISHED_SOAPSTONE)
                .add(ModBlocks.SMOOTH_SOAPSTONE)
                .add(ModBlocks.SOAPSTONE_BRICKS)
                .add(ModBlocks.SOAPSTONE_PILLAR)
                .add(ModBlocks.CHISELED_SOAPSTONE)
                .add(ModBlocks.LIMESTONE)
                .add(ModBlocks.POLISHED_LIMESTONE)
                .add(ModBlocks.SMOOTH_LIMESTONE)
                .add(ModBlocks.LIMESTONE_BRICKS)
                .add(ModBlocks.LIMESTONE_PILLAR)
                .add(ModBlocks.CHISELED_LIMESTONE)
                .add(ModBlocks.CINDER_SHARD_BLOCK)
                .add(ModBlocks.CINDER_CLUSTER)
                .add(ModBlocks.LARGE_CINDER_BUD)
                .add(ModBlocks.MEDIUM_CINDER_BUD)
                .add(ModBlocks.SMALL_CINDER_BUD)
                .add(ModBlocks.CINDER_SHARD_ORE)
                .add(ModBlocks.HELVINE_BLOCK)
                .add(ModBlocks.PEARL_BLOCK)
                .add(ModBlocks.ESPERITE_WALL)
                .add(ModBlocks.POLISHED_ESPERITE_WALL)
                .add(ModBlocks.MARBLE_WALL)
                .add(ModBlocks.POLISHED_MARBLE_WALL)
                .add(ModBlocks.SOAPSTONE_WALL)
                .add(ModBlocks.POLISHED_SOAPSTONE_WALL)
                .add(ModBlocks.LIMESTONE_WALL)
                .add(ModBlocks.POLISHED_LIMESTONE_WALL)
                .add(ModBlocks.FIRE_QUARTZ_BLOCK)
                .add(ModBlocks.SMOOTH_FIRE_QUARTZ_BLOCK)
                .add(ModBlocks.FIRE_QUARTZ_BRICKS)
                .add(ModBlocks.FIRE_QUARTZ_PILLAR)
                .add(ModBlocks.CHISELED_FIRE_QUARTZ)
                .add(ModBlocks.FIRE_QUARTZ_SLAB)
                .add(ModBlocks.FIRE_QUARTZ_STAIRS)
                .add(ModBlocks.FIRE_QUARTZ_ORE)
                .add(ModBlocks.ESPERITE_SLAB)
                .add(ModBlocks.POLISHED_ESPERITE_SLAB)
                .add(ModBlocks.ESPERITE_STAIRS)
                .add(ModBlocks.POLISHED_ESPERITE_STAIRS)
                .add(ModBlocks.MARBLE_SLAB)
                .add(ModBlocks.POLISHED_MARBLE_SLAB)
                .add(ModBlocks.MARBLE_STAIRS)
                .add(ModBlocks.POLISHED_MARBLE_STAIRS)
                .add(ModBlocks.SOAPSTONE_SLAB)
                .add(ModBlocks.POLISHED_SOAPSTONE_SLAB)
                .add(ModBlocks.SOAPSTONE_STAIRS)
                .add(ModBlocks.POLISHED_SOAPSTONE_STAIRS)
                .add(ModBlocks.LIMESTONE_SLAB)
                .add(ModBlocks.POLISHED_LIMESTONE_SLAB)
                .add(ModBlocks.LIMESTONE_STAIRS)
                .add(ModBlocks.POLISHED_LIMESTONE_STAIRS)
                .add(ModBlocks.POLISHED_HELVINE)
                .add(ModBlocks.LEMON_STONE_BLOCK)
                .add(ModBlocks.POLISHED_LEMON_STONE)
                .add(ModBlocks.TIGERS_EYE_BLOCK)
                .add(ModBlocks.POLISHED_TIGERS_EYE)
                .add(ModBlocks.KYANITE_BLOCK)
                .add(ModBlocks.POLISHED_KYANITE)
                .add(ModBlocks.PYRITE_BLOCK)
                .add(ModBlocks.POLISHED_PYRITE)
                .add(ModBlocks.FLUORAPATITE_BLOCK)
                .add(ModBlocks.POLISHED_FLUORAPATITE)
                .add(ModBlocks.JAGOITE_BLOCK)
                .add(ModBlocks.POLISHED_JAGOITE)
                .add(ModBlocks.CELESTINE_BLOCK)
                .add(ModBlocks.POLISHED_CELESTINE)
                .add(ModBlocks.GEMINITE_BLOCK)
                .add(ModBlocks.POLISHED_GEMINITE)
                .add(ModBlocks.JASPER_BLOCK)
                .add(ModBlocks.POLISHED_JASPER)
                .add(ModBlocks.AERINITE_BLOCK)
                .add(ModBlocks.POLISHED_AERINITE)
                .add(ModBlocks.ORPIMENT_BLOCK)
                .add(ModBlocks.POLISHED_ORPIMENT)
                .add(ModBlocks.DOLOMITE_BLOCK)
                .add(ModBlocks.POLISHED_DOLOMITE)
                .add(ModBlocks.JAMBORITE_BLOCK)
                .add(ModBlocks.POLISHED_JAMBORITE)
                .add(ModBlocks.NEKOITE_BLOCK)
                .add(ModBlocks.POLISHED_NEKOITE)
                .add(ModBlocks.NEPTUNITE_BLOCK)
                .add(ModBlocks.POLISHED_NEPTUNITE)
                .add(ModBlocks.SOAP_LIME_TILES)
                .add(ModBlocks.SOAP_ESPER_TILES)
                .add(ModBlocks.SOAP_MARBLE_TILES)
                .add(ModBlocks.MARBLE_LIME_TILES)
                .add(ModBlocks.MARBLE_ESPER_TILES)
                .add(ModBlocks.LIME_ESPER_TILES)
                .add(ModBlocks.LUNARITE_BLOCK)
                .add(ModBlocks.POLISHED_LUNARITE)
                .add(ModBlocks.END_LUNARITE_ORE)
                .add(ModBlocks.RAW_HEMATITE)
                .add(ModBlocks.REFINED_HEMATITE)
                .add(ModBlocks.HEMATITE_ROSE)
                .add(ModBlocks.DESERT_ROSE)
                .add(ModBlocks.MALACHITE_BLOCK)
                .add(ModBlocks.POLISHED_MALACHITE)
                .add(ModBlocks.CALCITE_MALACHITE_ORE)
                .add(ModBlocks.LIMESTONE_MALACHITE_ORE)
                .add(ModBlocks.OPAL_BLOCK)
                .add(ModBlocks.SMOOTH_OPAL_BLOCK)
                .add(ModBlocks.OPAL_BONE_BLOCK)
                .add(ModBlocks.OPAL_ORE)
                .add(ModBlocks.END_OPAL_ORE)
                .add(ModBlocks.DEEPSLATE_OPAL_ORE)
                .add(ModBlocks.FIRE_OPAL_BLOCK)
                .add(ModBlocks.SMOOTH_FIRE_OPAL_BLOCK)
                .add(ModBlocks.FIRE_OPAL_BONE_BLOCK)
                .add(ModBlocks.NETHER_FIRE_OPAL_ORE)
                .add(ModBlocks.END_FIRE_OPAL_ORE)
                .add(ModBlocks.DEEPSLATE_FIRE_OPAL_ORE)
                .add(ModBlocks.GEODE_ORE)
                .add(ModBlocks.DEEPSLATE_GEODE_ORE)
                .add(ModBlocks.FROZEN_GEODE_ORE)
                .add(ModBlocks.DEEPSLATE_FROZEN_GEODE_ORE)
                .add(ModBlocks.END_FROZEN_GEODE_ORE)
                .add(ModBlocks.NETHER_MAGMA_GEODE_ORE)
                .add(ModBlocks.BLACKSTONE_MAGMA_GEODE_ORE)
                .add(ModBlocks.BASALT_MAGMA_GEODE_ORE)
                .add(ModBlocks.DEEPSLATE_OMNI_GEODE_ORE)
                .add(ModBlocks.END_OMNI_GEODE_ORE)
                .add(ModBlocks.NETHER_OMNI_GEODE_ORE)
                .add(ModBlocks.BLACKSTONE_OMNI_GEODE_ORE)
                .add(ModBlocks.BASALT_OMNI_GEODE_ORE)
                .add(ModBlocks.FAIRY_STONE_BLOCK)
                .add(ModBlocks.END_FAIRY_ORE)
                .add(ModBlocks.BIXITE)
                .add(ModBlocks.POLISHED_BIXITE)
                .add(ModBlocks.TERRASINE_BLOCK)

                .add(ModBlocks.HELVINE_TILES)
                .add(ModBlocks.LEMON_STONE_TILES)
                .add(ModBlocks.TIGERS_EYE_TILES)
                .add(ModBlocks.KYANITE_TILES)
                .add(ModBlocks.PYRITE_TILES)
                .add(ModBlocks.FLUORAPATITE_TILES)
                .add(ModBlocks.JAGOITE_TILES)
                .add(ModBlocks.CELESTINE_TILES)
                .add(ModBlocks.GEMINITE_TILES)
                .add(ModBlocks.JASPER_TILES)
                .add(ModBlocks.AERINITE_TILES)
                .add(ModBlocks.ORPIMENT_TILES)
                .add(ModBlocks.DOLOMITE_TILES)
                .add(ModBlocks.JAMBORITE_TILES)
                .add(ModBlocks.NEKOITE_TILES)
                .add(ModBlocks.NEPTUNITE_TILES)

                .add(ModBlocks.CRYSTAL_COBBLESTONE)
                .add(ModBlocks.CRYSTAL_COBBLESTONE_STAIRS)
                .add(ModBlocks.CRYSTAL_COBBLESTONE_SLAB)
                .add(ModBlocks.CRYSTAL_COBBLESTONE_WALL)
                .add(ModBlocks.CRYSTAL_TILES)
        ;


        getOrCreateTagBuilder(BlockTags.NEEDS_IRON_TOOL)
                .add(ModBlocks.AQUAMARINE_BLOCK)
                .add(ModBlocks.AQUAMARINE_ORE)
                .add(ModBlocks.AQUAMARINE_DEEPSLATE_ORE)
                .add(ModBlocks.TOPAZ_BLOCK)
                .add(ModBlocks.TOPAZ_ORE)
                .add(ModBlocks.TOPAZ_DEEPSLATE_ORE)
                .add(ModBlocks.NETHER_TOPAZ_ORE)
                .add(ModBlocks.JADE_BLOCK)
                .add(ModBlocks.JADE_ORE)
                .add(ModBlocks.JADE_DEEPSLATE_ORE)
                .add(ModBlocks.RUBY_BLOCK)
                .add(ModBlocks.RUBY_ORE)
                .add(ModBlocks.RUBY_DEEPSLATE_ORE)
                .add(ModBlocks.NETHER_RUBY_ORE)
                .add(ModBlocks.CINDER_SHARD_ORE)
                .add(ModBlocks.FIRE_QUARTZ_ORE)
                .add(ModBlocks.RAW_HEMATITE)
                .add(ModBlocks.REFINED_HEMATITE)
                .add(ModBlocks.HEMATITE_ROSE)
                .add(ModBlocks.CALCITE_MALACHITE_ORE)
                .add(ModBlocks.LIMESTONE_MALACHITE_ORE)
                .add(ModBlocks.OPAL_ORE)
                .add(ModBlocks.END_OPAL_ORE)
                .add(ModBlocks.DEEPSLATE_OPAL_ORE)
                .add(ModBlocks.NETHER_FIRE_OPAL_ORE)
                .add(ModBlocks.END_FIRE_OPAL_ORE)
                .add(ModBlocks.DEEPSLATE_FIRE_OPAL_ORE)
        ;

        getOrCreateTagBuilder(BlockTags.NEEDS_DIAMOND_TOOL)
                .add(ModBlocks.NETHER_RADIONITE_ORE)
                .add(ModBlocks.RADIONITE_BLOCK)
                .add(ModBlocks.RAW_RADIONITE_BLOCK)
                .add(ModBlocks.PRISMATIC_SHARD_BLOCK)
                .add(ModBlocks.IRIDIUM_BLOCK)
                .add(ModBlocks.RAW_IRIDIUM_BLOCK)
                .add(ModBlocks.IRIDIUM_DEEPSLATE_ORE)
                .add(ModBlocks.NETHER_IRIDIUM_ORE)
                .add(ModBlocks.END_LUNARITE_ORE)
                .add(ModBlocks.FAIRY_STONE_BLOCK)
                .add(ModBlocks.END_FAIRY_ORE)
        ;

        getOrCreateTagBuilder(BlockTags.WALLS)
                .add(ModBlocks.ESPERITE_WALL)
                .add(ModBlocks.POLISHED_ESPERITE_WALL)
                .add(ModBlocks.MARBLE_WALL)
                .add(ModBlocks.POLISHED_MARBLE_WALL)
                .add(ModBlocks.SOAPSTONE_WALL)
                .add(ModBlocks.POLISHED_SOAPSTONE_WALL)
                .add(ModBlocks.LIMESTONE_WALL)
                .add(ModBlocks.POLISHED_LIMESTONE_WALL)
                .add(ModBlocks.CRYSTAL_COBBLESTONE_WALL)
        ;

        getOrCreateTagBuilder(BlockTags.FLOWERS)
                .add(ModBlocks.FAIRY_ROSE)
                .add(ModBlocks.BLUE_JAZZ)
                .add(ModBlocks.SUMMER_SPANGLE)
                .add(ModBlocks.DAFFODIL)
                .add(ModBlocks.SWEET_PEA)
                .add(ModBlocks.CROCUS)
                .add(ModBlocks.AMARANTH)
                .add(ModBlocks.FAIRY_ROSE_BUSH)
        ;

        getOrCreateTagBuilder(BlockTags.SMALL_FLOWERS)
                .add(ModBlocks.FAIRY_ROSE)
                .add(ModBlocks.BLUE_JAZZ)
                .add(ModBlocks.SUMMER_SPANGLE)
                .add(ModBlocks.DAFFODIL)
                .add(ModBlocks.SWEET_PEA)
                .add(ModBlocks.CROCUS)
        ;

        getOrCreateTagBuilder(BlockTags.TALL_FLOWERS)
                .add(ModBlocks.AMARANTH)
                .add(ModBlocks.FAIRY_ROSE_BUSH)
        ;

        getOrCreateTagBuilder(ModTags.Blocks.NEEDS_IRIDIUM_TOOL)
                .addTag(BlockTags.NEEDS_DIAMOND_TOOL);
        getOrCreateTagBuilder(ModTags.Blocks.NEEDS_NETHER_IRIDIUM_TOOL)
                .addTag(BlockTags.NEEDS_DIAMOND_TOOL);

        getOrCreateTagBuilder(BlockTags.LOGS_THAT_BURN)
                .add(ModBlocks.MAPLE_LOG)
                .add(ModBlocks.STRIPPED_MAPLE_LOG)
                .add(ModBlocks.MAPLE_WOOD)
                .add(ModBlocks.STRIPPED_MAPLE_WOOD)
                .add(ModBlocks.MYSTIC_LOG)
                .add(ModBlocks.STRIPPED_MYSTIC_LOG)
                .add(ModBlocks.MYSTIC_WOOD)
                .add(ModBlocks.STRIPPED_MYSTIC_WOOD)
                .add(ModBlocks.APRICOT_LOG)
                .add(ModBlocks.STRIPPED_APRICOT_LOG)
                .add(ModBlocks.APRICOT_WOOD)
                .add(ModBlocks.STRIPPED_APRICOT_WOOD)
                .add(ModBlocks.ORANGE_LOG)
                .add(ModBlocks.STRIPPED_ORANGE_LOG)
                .add(ModBlocks.ORANGE_WOOD)
                .add(ModBlocks.STRIPPED_ORANGE_WOOD)
                .add(ModBlocks.POMEGRANATE_LOG)
                .add(ModBlocks.STRIPPED_POMEGRANATE_LOG)
                .add(ModBlocks.POMEGRANATE_WOOD)
                .add(ModBlocks.STRIPPED_POMEGRANATE_WOOD)
                .add(ModBlocks.MANGO_LOG)
                .add(ModBlocks.STRIPPED_MANGO_LOG)
                .add(ModBlocks.MANGO_WOOD)
                .add(ModBlocks.STRIPPED_MANGO_WOOD)
                .add(ModBlocks.BANANA_LOG)
                .add(ModBlocks.STRIPPED_BANANA_LOG)
                .add(ModBlocks.BANANA_WOOD)
                .add(ModBlocks.STRIPPED_BANANA_WOOD)
                .add(ModBlocks.PEACH_LOG)
                .add(ModBlocks.STRIPPED_PEACH_LOG)
                .add(ModBlocks.PEACH_WOOD)
                .add(ModBlocks.STRIPPED_PEACH_WOOD)
        ;

        getOrCreateTagBuilder(ModTags.Blocks.MAPLE_LOGS)
                .add(ModBlocks.MAPLE_LOG)
                .add(ModBlocks.STRIPPED_MAPLE_LOG)
                .add(ModBlocks.MAPLE_WOOD)
                .add(ModBlocks.STRIPPED_MAPLE_WOOD)
        ;
        getOrCreateTagBuilder(ModTags.Blocks.MYSTIC_LOGS)
                .add(ModBlocks.MYSTIC_LOG)
                .add(ModBlocks.STRIPPED_MYSTIC_LOG)
                .add(ModBlocks.MYSTIC_WOOD)
                .add(ModBlocks.STRIPPED_MYSTIC_WOOD)
        ;
        getOrCreateTagBuilder(ModTags.Blocks.APRICOT_LOGS)
                .add(ModBlocks.APRICOT_LOG)
                .add(ModBlocks.STRIPPED_APRICOT_LOG)
                .add(ModBlocks.APRICOT_WOOD)
                .add(ModBlocks.STRIPPED_APRICOT_WOOD)
        ;
        getOrCreateTagBuilder(ModTags.Blocks.ORANGE_LOGS)
                .add(ModBlocks.ORANGE_LOG)
                .add(ModBlocks.STRIPPED_ORANGE_LOG)
                .add(ModBlocks.ORANGE_WOOD)
                .add(ModBlocks.STRIPPED_ORANGE_WOOD)
        ;
        getOrCreateTagBuilder(ModTags.Blocks.POMEGRANATE_LOGS)
                .add(ModBlocks.POMEGRANATE_LOG)
                .add(ModBlocks.STRIPPED_POMEGRANATE_LOG)
                .add(ModBlocks.POMEGRANATE_WOOD)
                .add(ModBlocks.STRIPPED_POMEGRANATE_WOOD)
        ;
        getOrCreateTagBuilder(ModTags.Blocks.MANGO_LOGS)
                .add(ModBlocks.MANGO_LOG)
                .add(ModBlocks.STRIPPED_MANGO_LOG)
                .add(ModBlocks.MANGO_WOOD)
                .add(ModBlocks.STRIPPED_MANGO_WOOD)
        ;
        getOrCreateTagBuilder(ModTags.Blocks.BANANA_LOGS)
                .add(ModBlocks.BANANA_LOG)
                .add(ModBlocks.STRIPPED_BANANA_LOG)
                .add(ModBlocks.BANANA_WOOD)
                .add(ModBlocks.STRIPPED_BANANA_WOOD)
        ;
        getOrCreateTagBuilder(ModTags.Blocks.PEACH_LOGS)
                .add(ModBlocks.PEACH_LOG)
                .add(ModBlocks.STRIPPED_PEACH_LOG)
                .add(ModBlocks.PEACH_WOOD)
                .add(ModBlocks.STRIPPED_PEACH_WOOD)
        ;
        getOrCreateTagBuilder(ModTags.Blocks.FIDDLEHEAD_LOGS)
                .add(ModBlocks.FIDDLEHEAD_LOG)
                .add(ModBlocks.STRIPPED_FIDDLEHEAD_LOG)
                .add(ModBlocks.FIDDLEHEAD_WOOD)
                .add(ModBlocks.STRIPPED_FIDDLEHEAD_WOOD)
        ;

        getOrCreateTagBuilder(BlockTags.FENCES)
                .add(ModBlocks.MAPLE_FENCE)
                .add(ModBlocks.MYSTIC_FENCE)
                .add(ModBlocks.APRICOT_FENCE)
                .add(ModBlocks.ORANGE_FENCE)
                .add(ModBlocks.POMEGRANATE_FENCE)
                .add(ModBlocks.MANGO_FENCE)
                .add(ModBlocks.BANANA_FENCE)
                .add(ModBlocks.PEACH_FENCE)
                .add(ModBlocks.FIDDLEHEAD_FENCE)
        ;
        getOrCreateTagBuilder(BlockTags.FENCE_GATES)
                .add(ModBlocks.MAPLE_FENCE_GATE)
                .add(ModBlocks.MYSTIC_FENCE_GATE)
                .add(ModBlocks.APRICOT_FENCE_GATE)
                .add(ModBlocks.ORANGE_FENCE_GATE)
                .add(ModBlocks.POMEGRANATE_FENCE_GATE)
                .add(ModBlocks.MANGO_FENCE_GATE)
                .add(ModBlocks.BANANA_FENCE_GATE)
                .add(ModBlocks.PEACH_FENCE_GATE)
                .add(ModBlocks.FIDDLEHEAD_FENCE_GATE)
        ;
        getOrCreateTagBuilder(BlockTags.PLANKS)
                .add(ModBlocks.APRICOT_PLANKS)
                .add(ModBlocks.ORANGE_PLANKS)
                .add(ModBlocks.PEACH_PLANKS)
                .add(ModBlocks.BANANA_PLANKS)
                .add(ModBlocks.MANGO_PLANKS)
                .add(ModBlocks.POMEGRANATE_PLANKS)
                .add(ModBlocks.FIDDLEHEAD_PLANKS)
                .add(ModBlocks.MAPLE_PLANKS)
                .add(ModBlocks.MYSTIC_PLANKS)
        ;
        getOrCreateTagBuilder(BlockTags.WOODEN_BUTTONS)
                .add(ModBlocks.APRICOT_BUTTON)
                .add(ModBlocks.ORANGE_BUTTON)
                .add(ModBlocks.PEACH_BUTTON)
                .add(ModBlocks.BANANA_BUTTON)
                .add(ModBlocks.MANGO_BUTTON)
                .add(ModBlocks.POMEGRANATE_BUTTON)
                .add(ModBlocks.FIDDLEHEAD_BUTTON)
                .add(ModBlocks.MAPLE_BUTTON)
                .add(ModBlocks.MYSTIC_BUTTON)
        ;
        getOrCreateTagBuilder(BlockTags.WOODEN_DOORS)
                .add(ModBlocks.APRICOT_DOOR)
                .add(ModBlocks.ORANGE_DOOR)
                .add(ModBlocks.PEACH_DOOR)
                .add(ModBlocks.BANANA_DOOR)
                .add(ModBlocks.MANGO_DOOR)
                .add(ModBlocks.POMEGRANATE_DOOR)
                .add(ModBlocks.FIDDLEHEAD_DOOR)
                .add(ModBlocks.MAPLE_DOOR)
                .add(ModBlocks.MYSTIC_DOOR)
        ;
        getOrCreateTagBuilder(BlockTags.WOODEN_FENCES)
                .add(ModBlocks.MAPLE_FENCE)
                .add(ModBlocks.MYSTIC_FENCE)
                .add(ModBlocks.APRICOT_FENCE)
                .add(ModBlocks.ORANGE_FENCE)
                .add(ModBlocks.POMEGRANATE_FENCE)
                .add(ModBlocks.MANGO_FENCE)
                .add(ModBlocks.BANANA_FENCE)
                .add(ModBlocks.PEACH_FENCE)
                .add(ModBlocks.FIDDLEHEAD_FENCE)
        ;
        getOrCreateTagBuilder(BlockTags.WOODEN_PRESSURE_PLATES)
                .add(ModBlocks.APRICOT_PRESSURE_PLATE)
                .add(ModBlocks.ORANGE_PRESSURE_PLATE)
                .add(ModBlocks.PEACH_PRESSURE_PLATE)
                .add(ModBlocks.BANANA_PRESSURE_PLATE)
                .add(ModBlocks.MANGO_PRESSURE_PLATE)
                .add(ModBlocks.POMEGRANATE_PRESSURE_PLATE)
                .add(ModBlocks.FIDDLEHEAD_PRESSURE_PLATE)
                .add(ModBlocks.MAPLE_PRESSURE_PLATE)
                .add(ModBlocks.MYSTIC_PRESSURE_PLATE)
        ;
        getOrCreateTagBuilder(BlockTags.WOODEN_SLABS)
                .add(ModBlocks.APRICOT_SLAB)
                .add(ModBlocks.ORANGE_SLAB)
                .add(ModBlocks.PEACH_SLAB)
                .add(ModBlocks.BANANA_SLAB)
                .add(ModBlocks.MANGO_SLAB)
                .add(ModBlocks.POMEGRANATE_SLAB)
                .add(ModBlocks.FIDDLEHEAD_SLAB)
                .add(ModBlocks.MAPLE_SLAB)
                .add(ModBlocks.MYSTIC_SLAB)
        ;
        getOrCreateTagBuilder(BlockTags.WOODEN_STAIRS)
                .add(ModBlocks.APRICOT_STAIRS)
                .add(ModBlocks.ORANGE_STAIRS)
                .add(ModBlocks.PEACH_STAIRS)
                .add(ModBlocks.BANANA_STAIRS)
                .add(ModBlocks.MANGO_STAIRS)
                .add(ModBlocks.POMEGRANATE_STAIRS)
                .add(ModBlocks.FIDDLEHEAD_STAIRS)
                .add(ModBlocks.MAPLE_STAIRS)
                .add(ModBlocks.MYSTIC_STAIRS)
        ;
        getOrCreateTagBuilder(BlockTags.WOODEN_TRAPDOORS)
                .add(ModBlocks.APRICOT_TRAPDOOR)
                .add(ModBlocks.ORANGE_TRAPDOOR)
                .add(ModBlocks.PEACH_TRAPDOOR)
                .add(ModBlocks.BANANA_TRAPDOOR)
                .add(ModBlocks.MANGO_TRAPDOOR)
                .add(ModBlocks.POMEGRANATE_TRAPDOOR)
                .add(ModBlocks.FIDDLEHEAD_TRAPDOOR)
                .add(ModBlocks.MAPLE_TRAPDOOR)
                .add(ModBlocks.MYSTIC_TRAPDOOR)
        ;
        getOrCreateTagBuilder(BlockTags.HOE_MINEABLE)
                .add(ModBlocks.APRICOT_LEAVES)
                .add(ModBlocks.ORANGE_LEAVES)
                .add(ModBlocks.PEACH_LEAVES)
                .add(ModBlocks.BANANA_LEAVES)
                .add(ModBlocks.MANGO_LEAVES)
                .add(ModBlocks.POMEGRANATE_LEAVES)
                .add(ModBlocks.FRUITING_APRICOT_LEAVES)
                .add(ModBlocks.FRUITING_ORANGE_LEAVES)
                .add(ModBlocks.FRUITING_PEACH_LEAVES)
                .add(ModBlocks.FRUITING_BANANA_LEAVES)
                .add(ModBlocks.FRUITING_MANGO_LEAVES)
                .add(ModBlocks.FRUITING_POMEGRANATE_LEAVES)
                .add(ModBlocks.FIDDLEHEAD_LEAVES)
                .add(ModBlocks.MAPLE_LEAVES)
                .add(ModBlocks.MYSTIC_LEAVES)
        ;
        getOrCreateTagBuilder(BlockTags.STANDING_SIGNS)
                .add(ModBlocks.STANDING_MAPLE_SIGN)
                .add(ModBlocks.STANDING_MYSTIC_SIGN)
                .add(ModBlocks.STANDING_APRICOT_SIGN)
                .add(ModBlocks.STANDING_ORANGE_SIGN)
                .add(ModBlocks.STANDING_POMEGRANATE_SIGN)
                .add(ModBlocks.STANDING_MANGO_SIGN)
                .add(ModBlocks.STANDING_BANANA_SIGN)
                .add(ModBlocks.STANDING_PEACH_SIGN)
                .add(ModBlocks.STANDING_FIDDLEHEAD_SIGN)
        ;
        getOrCreateTagBuilder(BlockTags.WALL_SIGNS)
                .add(ModBlocks.WALL_MAPLE_SIGN)
                .add(ModBlocks.WALL_MYSTIC_SIGN)
                .add(ModBlocks.WALL_APRICOT_SIGN)
                .add(ModBlocks.WALL_ORANGE_SIGN)
                .add(ModBlocks.WALL_POMEGRANATE_SIGN)
                .add(ModBlocks.WALL_MANGO_SIGN)
                .add(ModBlocks.WALL_BANANA_SIGN)
                .add(ModBlocks.WALL_PEACH_SIGN)
                .add(ModBlocks.WALL_FIDDLEHEAD_SIGN)
        ;
        getOrCreateTagBuilder(BlockTags.CEILING_HANGING_SIGNS)
                .add(ModBlocks.HANGING_MAPLE_SIGN)
                .add(ModBlocks.HANGING_MYSTIC_SIGN)
                .add(ModBlocks.HANGING_APRICOT_SIGN)
                .add(ModBlocks.HANGING_ORANGE_SIGN)
                .add(ModBlocks.HANGING_POMEGRANATE_SIGN)
                .add(ModBlocks.HANGING_MANGO_SIGN)
                .add(ModBlocks.HANGING_BANANA_SIGN)
                .add(ModBlocks.HANGING_PEACH_SIGN)
                .add(ModBlocks.HANGING_FIDDLEHEAD_SIGN)
        ;
        getOrCreateTagBuilder(BlockTags.WALL_HANGING_SIGNS)
                .add(ModBlocks.WALL_HANGING_MAPLE_SIGN)
                .add(ModBlocks.WALL_HANGING_MYSTIC_SIGN)
                .add(ModBlocks.WALL_HANGING_APRICOT_SIGN)
                .add(ModBlocks.WALL_HANGING_ORANGE_SIGN)
                .add(ModBlocks.WALL_HANGING_POMEGRANATE_SIGN)
                .add(ModBlocks.WALL_HANGING_MANGO_SIGN)
                .add(ModBlocks.WALL_HANGING_BANANA_SIGN)
                .add(ModBlocks.WALL_HANGING_PEACH_SIGN)
                .add(ModBlocks.WALL_HANGING_FIDDLEHEAD_SIGN)
        ;

    }
}
