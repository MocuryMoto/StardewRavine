...
public class HazelnutBushBlock extends PlantBlock implements Fertilizable {
    public static final MapCodec<HazelnutBushBlock> CODEC = createCodec(HazelnutBushBlock::new);
    ...
    @Override
    public MapCodec<HazelnutBushBlock> getCodec() {
        return CODEC;
    }

    public HazelnutBushBlock(Settings settings) {
        super(settings);
    ...
    protected VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        if ((Integer)state.get(AGE) == 0) {
            ...
            return state.get(AGE) < 5 ? LARGE_SHAPE : super.getOutlineShape(state, world, pos, context);
        }
    }
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (i > 4) {
            ...
            BlockState blockState = state.with(AGE, 3); //return to this age when picked
            world.setBlockState(pos, blockState, Block.NOTIFY_LISTENERS);
