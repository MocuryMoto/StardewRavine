...
public class SpiceBerryBushBlock extends PlantBlock implements Fertilizable {
    public static final MapCodec<SpiceBerryBushBlock> CODEC = createCodec(SpiceBerryBushBlock::new);
    ...
    @Override
    public MapCodec<SpiceBerryBushBlock> getCodec() {
        return CODEC;
    }

    public SpiceBerryBushBlock(Settings settings) {
        super(settings);
    ...
    public ItemStack getPickStack(WorldView world, BlockPos pos, BlockState state) {
        return new ItemStack(ModItems.SPICE_BERRY);
    }
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (i > 2) {
            ...
            dropStack(world, pos, new ItemStack(ModItems.SPICE_BERRY, j + (bl ? 1 : 0)));
            world.playSound(null, pos, SoundEvents.BLOCK_SWEET_BERRY_BUSH_PICK_BERRIES, SoundCategory.BLOCKS, 1.0F, 0.8F + world.random.nextFloat() * 0.4F);
            BlockState blockState = state.with(AGE, 1); //return to this age when picked
            world.setBlockState(pos, blockState, Block.NOTIFY_LISTENERS);
